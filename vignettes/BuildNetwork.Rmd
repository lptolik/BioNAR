...
##  Authors: Colin Mclean, Anatoly Sorokin, T. I. Simpson, J. Douglas Armstrong and Oksana Sorokina    

#Introduction

Most of proteomic studies usually result with a massive list of proteins being identified within a specific tissue, compartment or cell type, occasionally  accompanied by some qualitative or quantitative information. To obtain meaningful biological insight from this data, one usually needs to process the data in several stages to identify possible structural and/or functional dependencies.

One of the most popular ways of analysing the proteomic data is the building of the protein-protein interaction networks, to study how its topology influences functional annotation mapped onto it. 

Many existing packages support different steps of the network building and analysis process, but there are few packages that combine network building and analysis methodology into a single pipeline. 

We designed the BioNAR to combine the necessary network building and analysis functionality, complementing existing R packages and filling the methodological gaps necessary to interrogate biomedical networks with respect to functional and disease domains. For that purpose, we didn’t not implement the network reconstruction (unless it is synaptic network), given that other tools like Cytoscape and Network Analyst can do this already. Instead, we implemented a detailed topologically-based network analysis package, enabling the researcher to load networks generated from the lab’s own meta-data, thus making the tool as widely applicable and flexible as possible.

# Overview of capabilities
The package’s pipeline starts with loading/building the graph of interest and annotating its vertices with available metadata [annotate_vertex]. 
It follows with the analysis of general network properties, such as estimating a network’s “scale-free” property. For this we used the R “PoweRlaw” package (version 0.50.0) (Gillespie, 2015) [FitDegree] and a network entropy analysis (Teschendorff et al, 2014) [getEntopy].
Package allows estimating of the main network vertex centrality measures, such as degree, betweenness, clustering coefficient, semilocal centrality, mean shortest path, page rank, and standard deviation of the shortest path. The values for centrality values measures can be added as vertex node attributes [calcCentrality] or returned as a R matrix [getCentralityMatrix], depending on the user’s choice. Any other numerical characteristics, calculated for nodes and represented in a matrix form could be stored as node attributes as well.
To compare an observed networks vertex centrality values to those of an equivalently sized  randomised graph, we provide three varying randomisation models including G(n,p) Erdos-Renyi model , Barabasi-Albert model, and new random graph from a given graph by randomly adding/removing edges [getRandomGraphCentrality].
Additionally, to allow comparison of network with different structure, we implemented normalised modularity measure (Parter et al., 2007, Takemoto, 2012, Takemoto, 2013, Takemoto and Borjigin, 2011)[normModularity].

The pipeline then proceeds with uncovering the network’s community structure, which can be performed via nine different clustering algorithms, simultaneously [calcAllClustering] or with a chosen algorithm  [calcClustering], community membership being stored as a vertex attribute. In the situation when the network is dense and clusters are large and barely tractable, the reclustering can be applied [calcReclusterMatrix]. The obtained community structure can be visualized with [layoutByCluster] and further tested for robustness [getRobustness] by comparing with randomised networks. As a result, consensus matrix can be estimated [makeConsensusMatrix], which is needed for the next step -identifying the "influential" or "bridging" proteins (Nepusz et al., 2008).

For this, we enabled a function for estimating "bridgeness" [getBridgeness] metric, which takes into account the probability of the vertex to belong to different communities [getBridgeness], so that the vertices can be ranked under assumption that more communities the vertex can belong more influence it has to the network topology and signaling (Han et al., 2004). "Bridgeness" can be plot against any other centrality measure, e.g. semi-local centrality (plot is implemented), to enable additional view of vertex (protein) importance within the network topology.

To get a glance of the molecular signature of a disease or function on the network, we implemented the disease-disease overlap analysis by measuring the mean shortest distance for each disease (⟨d⟩), using the shortest distance between each gene-disease association (GDA) to its next nearest GDA neighbor (Menche et al., 2015) [calcDiseasePairs], [runPermDisease] to be used for obtaining signiificance values. In the presented example of the presynaptic network we found a few neurological disorders to overlap with a high significance, e.g. AD-PD, SCH-BP, ASD-ID, and, indeed, their comorbidity was confirmed by the literature. Same analysis can be performed with any in-house annotation.

To allow the distribution of the specific annotation(s) over clustered graph, we enabled overrepresentation analysis, which allows identify the network community enriched wit specific function or disease, or any other annotation.

The example illustrates the package functionality for the protein-protein interaction network for the presynaptic compartment of the synapse generated from Synaptic proteome database (Sorokina et al., 2021) with Synaptome.db package. The network has 1073 vertices and 3346 edges, the step by step analysis shown below.
...
Any predefined network stored as a graph file could be loaded for further analysis by use of igraph functionality.
...
As soon as graph is loaded it can be annotated with any relevant annotations, such as protein names, functionality, disease associations, or any customized annotation (annotate_vertex {BioNAR}. For example, below we use functional annotation based on two published synaptic studies.
## Gene name
...
Adding the annottation curated from (Lips et al., 2012).
```{r annotate_Shanno}
...
Adding the presynaptic genes functional annotation derived from (Boyken at al., 2019).
...
Evidence for scale-free structure can also be tested by performing a perturbation analysis on each network. In this analysis each protein is being perturbed through over-expression (red) and under-expression (green), with the global entropy rate (SR) after each proteins pertubation being plotted against the log of the proteins degree, as shown at the plot below. In the example network we observe a bi-modal response, between gene over-expression and degree, and opposing bi-phasic response relative to over/under-expression between global entropy rate and degree. This type of bi-modal, bi-phasic behaviour has been observed only in networks with scale-free or approximate scale-free topology (Teschendorff et al, 2014). 
