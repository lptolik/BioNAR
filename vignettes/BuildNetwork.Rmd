...
accompanied by some qualitative or quantitative information. To obtain 
meaningful biological insight from this data, one usually needs to process 
the data in several stages to identify possible structural and/or functional
dependencies.

One of the most popular ways of analyzing the proteomic data is the building 
...
analysis process, but few packages combine network building and analysis
methodology into a single pipeline. 

We designed BioNAR to combine the necessary network building and analysis
...
and disease domains. For that purpose, we did not implement network 
reconstruction (unless for synaptic networks), given that other tools like
...
BioNAR’s pipeline starts with loading and building the graph of interest, and
annotating its vertices with available metadata [annotate_vertex]. 

Thiis is followed by the analysis of general network properties, such as 
estimating a network’s “scale-free” property. For this we used the R 
“PoweRlaw” package (version 0.50.0) (Gillespie, 2015) [FitDegree] and a 
network entropy analysis
(Teschendorff et al, 2014) [getEntopy].

The package will allow estimates of the main network vertex centrality 
measures: degree, betweenness centrality, clustering coefficient, semilocal
centrality, mean shortest path, page rank, and standard deviation of the 
shortest path. Values for centrality values measures can be added as vertex
attributes [calcCentrality] or returned as an R matrix [getCentralityMatrix],
depending on the users choice. Any other vertex meta-data, which can be 
represented in matrix form, can also be stored as a vertex attribute as well.
...
Additionally, to allow comparison of networks with different structures, 
we implemented normalised modularity measure (Parter et al., 2007, 
Takemoto, 2012, Takemoto, 2013, Takemoto and Borjigin, 2011)[normModularity].

BioNAR then proceeds with uncovering the network’s community structure, which 
can be performed via nine different clustering algorithms, simultaneously
[calcAllClustering] or with a chosen algorithm  [calcClustering], community
membership being stored as a vertex attribute. In situations when the network 
...
can be plot against any other centrality measure, e.g. semi-local centrality 
(plot is implemented), to enable additional view of vertex (protein) importance
...
obtaining signiificance values. In the case example of the presynaptic 
network we found a set of neurological disorders to overlap with a high
...
we enabled overrepresentation analysis, which helps identify the network
communities enriched for specific function or disease, or any other annotation.
...
BioNAR allows building a network from a data frame, where the rows correspond 
to the edges of the graph, or for the list of vertices (genes), for which the
information will be retrieved from SynaptomeDB package.

## Build the network from data frame
The command builds a graph from provided data frame, simplifies the graph 
...
The command provides proxy for Synaptome.db package, enabling building the protein-protein interaction (PPI) network for entire synaptic compartment 
...
annotations, such as protein names, functionality, disease associations, or 
any customized annotation set [annotate_vertex {BioNAR}]. For example, below 
we use functional annotation based on two published synaptic studies.
...
you prefer to provide different names for the vertex centrality attributes, 
the same centrality measures can be obtained in a matrix form:
...
Sometimes one needs to compare the graph properties of the the properties of 
an the observed network to randomised networks of a similar size. The BioNAR
...
Barabasi-Albert model and new random graph from a given graph by randomly 
...
To examine a network's underlying structure (i.e. not random), one can test the
...
“PoweRlaw” package (version 0.50.0) (Gillespie, 2015). For the case study, 
i.e. our presynaptic PPI network, we found evidence for disassortative mixing
...
In our case study of the presynaptic PPI network we observe a bi-modal 
...
degree. This type of bi-modal, bi-phasic behaviour has been observed only in
networks with scale-free or approximate scale-free topology 
...
Normalised modularity (Qm) allows the comparison of networks with 
varying structure. Qm based on the previous studies by Parter et al., 2007,
Takemoto, 2012, Takemoto, 2013, Takemoto and Borjigin, 2011, which was 
defined as:

$$Qm = \frac{Q_{real}-Q_{rand}}{Q_{max}-Q_{rand}}$$

Where $Q_{real}$ is the network modularity of a real-world signaling 
network and, $Q_{rand}$ is the average network modularity value obtained 
...
as a community (sub-complex, module or cluster) in PPI networks. The possible
...
For each algorithm of interest the community membership can be obtained
with`calcMembership` command. 
...
algorithms were chosen as to maximise the measure Modularity (Newman & Girvan,
2004); infomap seeks the optimal community structure in the data by maximising 
the objective function called the Minimum Description 
...
functions `calcClustering` and `calcAllClustering` that use calcMembership 
to calculate community memberships and store them within the graph vertices 
attributes named after the algorithm. They also calculate modularity values and
store them as graph vertex attributes named after the clustering algorithm. The
difference between `calcClustering` and `calcAllClustering` is that
...
indistinguishable within the central part of the plot. So we could layout graph
clusterwise:
...
performed on the community set found, 'Cnmin' minimum cluster size and 'Cnmax' 
the maximum cluster size above which reclustering will be preformed (if
reClust=TRUE). 
```{r cons_mat}
...
single community, and 1 - implying a vertex forms a 'global bridge' across 
...
diseases: DOID:10652 (Alzheimer's_disease),  (bipolar_disorder), DOID:12849
(autistic_disorder), DOID:1826 (epilepsy). Command `calcDiseasePairs` quickly
estimates the two annotation separation on the graph and compares it with one
randomly reannotated graph. This could be 
...
```
```{r pdisPairs,warning=FALSE,message=FALSE}
r <- runPermDisease(
    gg,
    name = "TopOnto_OVG_HDO_ID",
    diseases = c("DOID:10652","DOID:3312"),
